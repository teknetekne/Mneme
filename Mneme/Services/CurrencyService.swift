...
actor CurrencyService {
    ...
    private var inFlightTask: Task<[String: Double]?, Never>?

    // Rate limit backoff: skip API calls until this date
    ...
    func getExchangeRate(from: String, to: String) async -> Double? {
        ...
        if usdRates == nil {
            if let rates = await deduplicatedFetchRates() {
                 // Save to Core Data
    ...
    private func deduplicatedFetchRates() async -> [String: Double]? {
            ...
            if let jsonData = try? JSONSerialization.data(withJSONObject: rates, options: []),
               let jsonString = String(data: jsonData, encoding: .utf8) {
                ...
                do {
                    ...
                    print("CurrencyService save error: \(error)")
                }
            }
        }
    }
